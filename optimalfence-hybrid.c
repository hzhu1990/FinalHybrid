...
bool CalcularCombinacionOptima(int PrimeraCombinacion, int UltimaCombinacion, PtrListaArboles Optimo)
{
	...
	int num_threads = 4;
	int idxStart, idxEnd, nCombinations;
	int slaveResult[2];
	...
	// Divide the data (MPI)
	nCombinations = UltimaCombinacion - PrimeraCombinacion;
	idxStart = (nCombinations / numtasks) * rank + PrimeraCombinacion;
	idxEnd = rank != numtasks - 1 ? (nCombinations / numtasks) * (rank + 1) + PrimeraCombinacion : UltimaCombinacion;

	// Divide the data (OpenMP)
	int workload = (idxEnd - idxStart) / (num_threads * 20);
	workload = workload == 0 ? 1 : workload;

	CosteMejorCombinacion = Optimo->Coste;

	#pragma omp parallel for schedule(static, workload)
	for (Combinacion = idxStart; Combinacion < idxEnd; Combinacion++)
	{
		int Coste = EvaluarCombinacionListaArboles(Combinacion);
	...
	// Put suboptimals in common
	if (rank == 0) {
		// recv best combinations of workers
		int i;
		for (i = 1; i < numtasks; i++) {
			MPI_Status status;
			MPI_Recv(slaveResult, 2, MPI_INT, MPI_ANY_SOURCE, 42, MPI_COMM_WORLD, &status);
			if (slaveResult[0] < CosteMejorCombinacion) {
				CosteMejorCombinacion = slaveResult[0];
				MejorCombinacion = slaveResult[1];
			}
		}
	} else {
		// send best combination to 0
		slaveResult[0] = CosteMejorCombinacion;
		slaveResult[1] = MejorCombinacion;
		MPI_Send(slaveResult, 2, MPI_INT, 0, 42, MPI_COMM_WORLD);
	}
